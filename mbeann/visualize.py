'''
Visualization tools for MBEANN in Python.
...
def visualizeIndividual(ind, fileName):
    ...
    # edges = nx.draw_networkx_edges(G, pos, arrowstyle="->", arrowsize=10, edge_color=edge_colors, edge_cmap=plt.cm.coolwarm, edge_vmin=ind.minWeight, edge_vmax=ind.maxWeight)
    edges_pos = nx.draw_networkx_edges(G, pos, edgelist=edge_pos, arrowstyle='->', arrowsize=10,
                                       width=weights_pos, edge_color='tab:blue')
    edges_neg = nx.draw_networkx_edges(G, pos, edgelist=edge_neg, arrowstyle='->', arrowsize=10,
                                       width=weights_neg, edge_color='tab:orange', style='--')
